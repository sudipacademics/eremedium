...
wait_dns() {
  local host=$1
  local ip
  ip=$(dig +short "$host" A | tail -n1 | tr -d '[:space:]')
  echo "$host -> ${ip:-NONE}"
  [[ "$ip" == "$TARGET_IP" ]]
}
...
# Always install bootstrap so ACME path exists once DNS lands
ln -sfn "$NGINX_SRC/vedsutra-bootstrap.conf" /etc/nginx/sites-enabled/zz-vedsutra.in
# Keep shared upstreams via astro.conf (redirect edition from this branch)
...
if [[ "$DNS_OK" != "1" ]]; then
  echo "DNS_NOT_READY"
  echo "Set webhostbox A @ and www to $TARGET_IP, then re-run this script."
  echo "Bootstrap is live: http://$DOMAIN/ should hit this host once DNS propagates."
  exit 2
fi
...
for line in text.splitlines():
    ...
    if key in repl:
        ...
        # rewrite any remaining old hostname in values
        lines.append(line.replace("astro.e-remedium.in", "vedsutra.in"))
for key, val in repl.items():
    if key not in seen:
        lines.append(f"{key}={val}")
p.write_text("\n".join(lines) + "\n")
print("env updated:", ", ".join(sorted(repl)))
PY
...
curl -fsS -o /dev/null -w 'vedsutra:%{http_code}\n' "${AUTH[@]}" "https://vedsutra.in/" || true
curl -fsS -o /dev/null -w 'www:%{http_code} redirect:%{redirect_url}\n' "${AUTH[@]}" "https://www.vedsutra.in/" || true
curl -fsS -o /dev/null -w 'astro:%{http_code} redirect:%{redirect_url}\n' "${AUTH[@]}" "https://astro.e-remedium.in/" || true
