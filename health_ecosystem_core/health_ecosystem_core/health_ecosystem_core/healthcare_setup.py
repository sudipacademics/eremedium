    ...
    for user_name in frappe.get_all("User", filters={"enabled": 1}, pluck="name"):
        ...
        if "default_workspace" in user_fields:
            ...
            if any(r in roles for r in ("Healthcare Administrator", "Health System Admin", "Physician")):
                ...
                elif frappe.db.get_value("User", user_name, "default_workspace") == "Healthcare":
                    updates["default_workspace"] = _fallback_workspace()
...
\tvar slugIdx = segments.indexOf("{page_slug}");
\tvar tail = slugIdx >= 0 && segments.length > slugIdx + 1
\t\t? decodeURIComponent(segments[slugIdx + 1])
\t\t: null;
\tif (tail === "new") {{
\t\tfrappe.new_doc("{doctype}");
\t}} else if (tail) {{
\t\tfrappe.set_route("Form", "{doctype}", tail);
\t}} else {{
...
def fix_doctype_desk_routes():
    """Set DocType.route so Marley-generated links use short slugs."""
    if not frappe.db.has_column("DocType", "route"):
        return {}
    changed = {}
    for doctype, route in HEALTHCARE_PAGE_SLUGS.items():
        if not frappe.db.exists("DocType", doctype):
            continue
        current = frappe.db.get_value("DocType", doctype, "route")
        if current != route:
            frappe.db.set_value("DocType", doctype, "route", route, update_modified=False)
            changed[doctype] = route
    return changed
...
def _fallback_workspace():
    for name in ("Healthcare", "Home", "ERPNext"):
...
def ensure_healthcare_workspace():
    """Reload Healthcare workspace from Marley app JSON if missing."""
    if "healthcare" not in (frappe.get_installed_apps() or []):
        return {"ok": False, "error": "healthcare app not installed"}

    if frappe.db.exists("Workspace", "Healthcare"):
        return {"ok": True, "workspace": "Healthcare", "action": "exists"}
    ...
    try:
        ...
        frappe.log_error(title="sync_healthcare_workspace", message=frappe.get_traceback())
    ...
    for ws in ("healthcare", "Healthcare"):
        try:
            ...
            try:
                frappe.reload_doc("healthcare", "workspace", ws.capitalize())
            except Exception:
                pass

    frappe.db.commit()
    exists = frappe.db.exists("Workspace", "Healthcare")
    return {
        "ok": bool(exists),
        "workspace": "Healthcare" if exists else None,
        "action": "reloaded" if exists else "missing_after_reload",
    }


def fix_healthcare_default_workspace():
    """
    Fix 'Could not find Default Workspace: Healthcare'.
    Reload workspace from Marley JSON, then reset users if still missing.
    """
    ws_result = ensure_healthcare_workspace()
    ...
    if frappe.db.has_column("User", "default_workspace"):
        for user_name in frappe.get_all(
            "User",
            filters={"enabled": 1, "default_workspace": "Healthcare"},
            pluck="name",
        ):
            if frappe.db.exists("Workspace", "Healthcare"):
                continue
