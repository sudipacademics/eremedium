...
import { RFMS_API_BASE, parseAdminNotificationHref, type AdminNavigationTarget } from '@rfms/utils';
...
export function NotificationBell({
  ...
  token: string;
  onNavigate: (target: AdminNavigationTarget) => void;
}) {
  ...
  async function openItem(item: NotificationItem) {
    ...
    const target = parseAdminNotificationHref(item.href);
  ...
  return (
    <div className="notification-shell" ref={rootRef}>
      <button type="button" className="bell" aria-expanded={open} onClick={() => setOpen((value) => !value)}>
        Alerts{unreadCount ? <i>{unreadCount}</i> : null}
      </button>
      {open ? (
...
export function ProfileMenu({
  ...
  return (
    <div className="profile-menu-shell" ref={rootRef}>
      <button type="button" className="profile" aria-expanded={open} onClick={() => setOpen((value) => !value)}>{initials}</button>
      {open ? (
