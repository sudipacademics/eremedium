...
const subdivisionsByDistrict: Record<string, string[]> = {
  Alipurduar: ['Alipurduar', 'Falakata'], Bankura: ['Bankura Sadar', 'Khatra'], Birbhum: ['Bolpur', 'Rampurhat', 'Suri Sadar'],
  'Cooch Behar': ['Cooch Behar Sadar', 'Dinhata', 'Mathabhanga', 'Mekhliganj', 'Tufanganj'], 'Dakshin Dinajpur': ['Balurghat', 'Gangarampur'],
  Darjeeling: ['Darjeeling Sadar', 'Kurseong', 'Mirik', 'Siliguri'], Hooghly: ['Arambagh', 'Chandannagore', 'Chinsurah', 'Srirampore'],
  Howrah: ['Howrah Sadar', 'Uluberia'], Jalpaiguri: ['Jalpaiguri Sadar', 'Malbazar'], Jhargram: ['Jhargram'], Kalimpong: ['Kalimpong'], Kolkata: ['Kolkata Sadar'],
  Malda: ['Chanchal', 'Malda Sadar'], Murshidabad: ['Berhampore', 'Domkal', 'Jangipur', 'Kandi', 'Lalbagh'], Nadia: ['Kalyani', 'Krishnanagar Sadar', 'Ranaghat', 'Tehatta'],
  'North 24 Parganas': ['Barasat', 'Barrackpore', 'Bashirhat', 'Bidhannagar'], 'Paschim Bardhaman': ['Asansol', 'Durgapur'], 'Paschim Medinipur': ['Ghatal', 'Kharagpur', 'Medinipur Sadar'],
  'Purba Bardhaman': ['Bardhaman Sadar North', 'Bardhaman Sadar South', 'Kalna', 'Katwa'], 'Purba Medinipur': ['Contai', 'Egra', 'Haldia', 'Tamluk'],
  Purulia: ['Manbazar', 'Purulia Sadar'], 'South 24 Parganas': ['Alipore Sadar', 'Baruipur', 'Canning', 'Diamond Harbour', 'Kakdwip'], UttarDinajpur: ['Islampur', 'Raiganj'],
};
const districts = Object.keys(subdivisionsByDistrict).map((key) => key === 'UttarDinajpur' ? 'Uttar Dinajpur' : key).sort((a, b) => a.localeCompare(b));
const subdivisionsFor = (district: string) => subdivisionsByDistrict[district] ?? (district === 'Uttar Dinajpur' ? subdivisionsByDistrict.UttarDinajpur : []);
...
function GoogleTerritoryMap({ territories, selectedId, onSelect }: { territories: Territory[]; selectedId: string; onSelect: (id: string) => void }) {
  ...
  useEffect(() => {
    ...
    async function renderTerritories() {
      ...
      const entries = territories.flatMap((territory) => territory.pin_capacities.map((pin) => ({ territory, pin })));
  ...
  }, [ready, territories, selectedId, onSelect]);

  return <section className="territory-map-card google-territory-map" aria-labelledby="west-bengal-map-title">
    <div className="territory-map-head"><div><p className="territory-kicker">Live territory coverage</p><h2 id="west-bengal-map-title">West Bengal territory map</h2><p>Live Google Maps pins use each territory’s PIN code and West Bengal address. Use the native controls, mouse wheel or pinch gestures to inspect an exact location.</p></div><span className="google-map-live">Live Google Map</span></div>
    ...
    <div className="territory-map-legend"><span><i className="available" /> Fully available</span><span><i className="reserved" /> Partly available / fully reserved</span><span><i className="occupied" /> Fully occupied</span><small>Markers are PIN-code locations. Rings appear for the selected territory: inner = FOFO radius, outer = FOCO radius.</small></div>
  </section>;
}

function TerritoryMap(props: { territories: Territory[]; selectedId: string; onSelect: (id: string) => void }) {
  return GOOGLE_MAPS_API_KEY ? <GoogleTerritoryMap {...props} /> : <StaticWestBengalMap {...props} />;
}
...
export function TerritorySetup({ token, search, notify }: { token: string; search: string; notify: (message: string) => void }) {
  ...
  const visibleTerritories = useMemo(() => { const query = search.trim().toLowerCase(); return query ? territories.filter((territory) => `${territory.state} ${territory.district} ${territory.subdivision} ${territory.area} ${territory.pincodes.join(' ')}`.toLowerCase().includes(query)) : territories; }, [search, territories]);
  ...
  const subdivisions = draft.district ? subdivisionsFor(draft.district) : [];
  ...
  const addPincode = () => { const pincode = draft.pinEntry.replace(/\D/g, '').slice(0, 6); if (!/^\d{6}$/.test(pincode)) { setError('Enter a six-digit PIN code, then choose Add PIN.'); return; } if (draft.pins.some((pin) => pin.pincode === pincode)) { setError('This PIN code is already added.'); return; } setDraft((current) => ({ ...current, pins: [...current.pins, { pincode, fofo_available: '0', foco_available: '0' }], pinEntry: '' })); setError(''); };
